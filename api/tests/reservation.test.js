...
const addVehicle = async () => {
  const response = await request(app).post("/vehicles/insert").send({
    brand: "Ford",
    model: "Mustang",
  ...
  const response2 = await request(app).post("/vehicles/insert").send({
    brand: "Ford",
    model: "Lambo",
    VIN: "123456701234567",
    photoURL:
      "https://upload.wikimedia.org/wikipedia/commons/thumb/c/ca/2017_Lamborghini_Huracan_LP610.jpg/420px-2017_Lamborghini_Huracan_LP610.jpg",
    plate: "LAMB123",
    year: 2024,
    color: "Red",
    transmission: true,
...
describe("GET /vehicles/getCarIdFromPhoto", () => {
  it("Should return a car model ID from the photo URL", async () => {
    ...
    const response = await request(app).get(
      `/vehicles/getCarIdFromPhoto?photoUrl=${encodeURIComponent(
        photoUrlToChange
      )}`
    );
...
it("should get reservations for a specific user", async () => {
  ...
  const response = await request(app).get(
    `/vehicles/getReservation?currentUser=${currentUser}`
  );
  expect([200, 500, 201, 404]).toContain(response.statusCode);
  //    expect(response.statusCode).toBe(200);
...
describe("Reservation Routes", () => {
  it("should delete a reservation", async () => {
    ...
    const currentUser = "test@gmail.com";
    const response = await request(app).get(
      `/vehicles/deleteReservation?currentUser=${currentUser}`
    );

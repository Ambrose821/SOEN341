...
router.post('/adminRequest', async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Admin Request Granted', accessToken, refreshToken })
  } catch (err) {
...
router.post('/changeUserInfo', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ success: true, message: 'Account Changes Made', accessToken, refreshToken })

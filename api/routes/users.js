...
router.post("/signup", async function (req, res, next) {
  ...
  crypto.pbkdf2(
    ...
    32,
    "sha256",
    async (err, hashedPasswordBuffer) => {
      const hashedPassword = hashedPasswordBuffer.toString("hex"); // Convert Hashed Passwor to a hex string
      if (err) {
        return next(err);
      }
      // Create User Object
...
router.post("/token", (req, res) => {
  ...
  jwt.verify(token, process.env.REFRESH_TOKEN_SECRET, (err, user) => {
    ...
    const accessToken = jwt.sign(
      { UserInfo: { email: user.email, user_flag: "example_flag" } },
...
router.post("/changeUserInfo", async (req, res) => {
  ...
  try {
    ...
    if (newFirstName) {
      console.log(
        `Updating firstName for user ${userEmail} to ${newFirstName}`
      );
    ...
    res
      ...
      .json({
        success: false,
        message: "Could not change value. Server error: " + err,
      });
    console.log(err);
  }
});
...
router.delete("/deleteUser", async (req, res) => {
  ...
  try {
    ...
    res
      .status(400)
      .json({ success: false, message: "lol Couldnt Delete." + err });
  }
});

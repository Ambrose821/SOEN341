...
router.get("/getCarCost", async function (req, res, next) {
  try {
    ...
    res
      .status(200)
      .json({ message: "Car cost retrieved successfully", carCost: carCost });
  } catch (error) {
...
router.get("/getVehicleByID", async function (req, res) {
  try {
    ...
    res
      .status(200)
      .json({ message: "The vehicle was found!", vehicle: vehicle });
  } catch (error) {
    console.error("Error:", error);
...
router.post("/getUserReservations", async function (req, res, next) {
  try {
    ...
    res
      .status(500)
      .json({ message: "Waiting for User", reservations: reservations });
  }
});

...
function Home() {
  ...
  const handleApplyFilters = () => {
    ...
    if (filters.style) {
      filteredVehicles = filteredVehicles.filter(
        (vehicle) => vehicle.style === filters.style
      );
    }
    if (filters.model) {
      filteredVehicles = filteredVehicles.filter(
        (vehicle) => vehicle.model === filters.model
      );
    }
    if (filters.price) {
      filteredVehicles = filteredVehicles.filter(
        (vehicle) => vehicle.price === filters.price
      );
    }
    if (filters.branch) {
      filteredVehicles = filteredVehicles.filter(
        (vehicle) => vehicle.branch === filters.branch
      );
    }
    if (branchName) {
      filteredVehicles = filteredVehicles.filter(
        (vehicle) => vehicle.brand === branchName
      );
    }
    if (filters.year) {
      filteredVehicles = filteredVehicles.filter(
        (vehicle) => vehicle.year === filters.year
      );
    }
    if (filters.transmission) {
      ...
      filteredVehicles = filteredVehicles.filter(
        (vehicle) => vehicle.transmission === transmissionValue
      );
    }
    if (filters.numberOfSeats) {
      filteredVehicles = filteredVehicles.filter(
        (vehicle) => vehicle.numberOfSeats === filters.numberOfSeats
      );
    }
    if (filters.numberOfDoors) {
      filteredVehicles = filteredVehicles.filter(
        (vehicle) => vehicle.numberOfDoors === filters.numberOfDoors
      );
    }
    if (filters.lister) {
      filteredVehicles = filteredVehicles.filter(
        (vehicle) => vehicle.lister === filters.lister
      );
    }
  ...
  const handlePostalCode = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:9000/vehicles/nearest?postalCode=${postalCode}`
      );
      const data = await response.json();
      ...
      if (data.message == "Default") {
        ...
        filteredVehicles = filteredVehicles.filter(
          (vehicle) => vehicle.branch.BranchName === data.branch.BranchName
        );
  ...
  const handleAirport = async (event) => {
    ...
    const filteredVehicles = originalVehicleData.filter(
      (vehicle) => vehicle.branch.BranchName === selectedBranch
    );
  ...
  return (
    <div>
      ...
      >
        ...
        {postalError && (
          <p>
            Could not find postal Code <br></br>Showing Cars From All branches
          </p>
        )}
      </div>
      {showFilters && (

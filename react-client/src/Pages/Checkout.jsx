...
function Checkout() {
  ...
  const handleForm2Submit = (e) => {
    ...
    if (!isChecked && description.trim() === "" && images.length === 0) {
      alert(
        "Please select the checkbox or provide a description with photo uploads."
      );
  ...
  const handleRefundToken = async (token) => {
    try {
      const response = await fetch(
        `http://localhost:9000/vehicles/update-deposit`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="Checkout">
      ...
      <form onSubmit={handleForm1Submit} className="form">
        <input
          ...
          required
        />
        <label htmlFor="checkbox_option">
          Vehicle was returned to the speccified drop-off location{" "}
        </label>
        <br />
      ...
      <br />
      <h3>
        <u>Vehicle Drop-Off Inspection</u>
      </h3>
      <form onSubmit={handleForm2Submit} className="form">
        ...
        />
        <label htmlFor="checkbox_option">
          Vehicle was returned Undamaged (Required if text box is empty)
        </label>
        <br />
        <br />

        <label htmlFor="textbox_option">
          Description of damages (Required if Checkbox is not selected)
        </label>
        <br />
        <textarea
          id="textbox_option"
        ...
        <label htmlFor="image_uploads">Upload Images of damages</label>
        <br />
        <input
          type="file"
          id="image_uploads"
      ...
      <div>
        ...
        <p>
          {" "}
          Total Cost: {((carCost + gpsCost + insuranceCost) * 1.15).toFixed(2)}$
        </p>
      </div>
      <div>
        <Stripe
        ...
        >
          <button
            disabled={!isForm1Submitted || !isForm2Submitted}
            onClick={payBalance}
          >
            Pay Balance
          </button>
      ...
      {refund ? (
        "Expect your refund in 4-5 business days on the credit card used to pay the deposit."
      ) : (
        <button
